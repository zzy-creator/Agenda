...
/**
   ...
   * @param admin the meeting's  administrator to be set
   * @return if success, true will be returned
   */
bool AgendaService::setAdmin(const std::string& userName, const std::string admin, const std::string& title) {
	if (userName == admin) return false;//管理员不能为发起者
	auto f = [userName, title,admin](const Meeting& a)->bool {
		if (a.getSponsor() == userName && a.getTitle() == title&&a.isParticipator(admin)&&a.getAdmin().length()==0) return true;
		else return false;
	};
	auto s = [admin](Meeting& a) {
		a.setAdmin(admin);
	};
 ...
 */
  ...
  bool AgendaService::addMeetingParticipator(const std::string& userName,
	  ...
	  auto f = [userName, title](const Meeting& a)->bool {
		  if ((a.getSponsor() == userName||a.getAdmin()==userName) && a.getTitle() == title) return true;
		  else return false;
	  };
  ...
  bool AgendaService::removeMeetingParticipator(const std::string& userName,
	  ...
	  auto f = [userName, title,participator](const Meeting& a)->bool {
		  if ((userName==a.getSponsor()||a.getAdmin()==userName) && a.getTitle() == title && a.isParticipator(participator) )return true;
		  else return false;
	  };
